...
def render_template(request: web.Request, template: str, variables: dict[str, Any] = {}) -> web.Response:
    """
...
def date_delta(before: datetime.date, after: datetime.date) -> DateDelta:
    ...
    and days.
    """
    delta_months = ((after.year * 12) + after.month) - ((before.year * 12) + before.month)
...
def get_memory(cur: sqlite3.Cursor, id: int) -> list[Memory]:
    ...
    return Memory(
        ...
        date=date,
        name=name,
        age=date_delta(datetime.date.fromisoformat(dob), datetime.date.fromisoformat(date)),
        note=note,
    )


def group_memories(memories: list[Memory]) -> OrderedDict[tuple[int, int], list[Memory]]:
    """
...
async def post_memories(request: web.Request) -> web.Response:
    ...
    if "date" in data and "note" in data:
        ...
        if not re.match(r"[0-9]{4}-[0-9]{2}-[0-9]{2}", date):
            raise web.HTTPBadRequest(text=f"Invalid date: {date!r}, expected YYYY-MM-DD")
        if not note.strip():
        ...
        if id is None:
            ...
            res = cur.execute("UPDATE memories SET date = ?, note = ? WHERE id = ?", (date, note, id))
        ...
        raise web.HTTPSeeOther(f"{request.url.path}#age-{memory.age.years}-{memory.age.months}")
